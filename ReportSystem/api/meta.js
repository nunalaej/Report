// api/meta.js
import MetaConfig from '../models/MetaConfig'; // Ensure the MetaConfig model exists
import dotenv from 'dotenv';

dotenv.config();

const DEFAULT_BUILDINGS = [
  'Ayuntamiento', 'JFH', 'ICTC', 'PCH', 'Food Square', 'COS', 'CBAA', 'CTHM', 'GMH', 'CEAT', 'Other',
];
...
export default async (req, res) => {
  if (req.method === 'GET') {
    try {
      ...
      res.json({ buildings: doc.buildings || DEFAULT_BUILDINGS, concerns: doc.concerns || DEFAULT_CONCERNS });
    } catch (err) {
      console.error('Get meta error:', err);
      res.json({ buildings: DEFAULT_BUILDINGS, concerns: DEFAULT_CONCERNS });
    }
  }
  ...
  if (req.method === 'PUT') {
    try {
      ...
      res.json({ success: true, buildings: doc.buildings, concerns: doc.concerns });
    } catch (err) {
