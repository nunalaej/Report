...
const deriveFloorFromRoom = (roomValue) => {
  if (!roomValue) return "";
  const num = parseInt(roomValue, 10);
...
export default function Create() {
  ...
  useEffect(() => {
    try {
      ...
      if (u?.kind === "user" && u.email) {
        setCurrentUserEmail(u.email);
        setFormData((f) => ({ ...f, email: u.email }));
      }
    } catch {
      // ignore
    }
  ...
  const floorRoomRanges = {
    "First Floor": ["101-110"],
    "Second Floor": ["201-213"], // include up to 213
    "Third Floor": ["301-310"],
    ...
    try {
      ...
      if (result.success) {
        if (result.report && typeof result.report === "object") {
          setExistingReports((prev) => [...prev, result.report]);
        }
  ...
  const ictcFirstFloor =
    ictcHasSpecific && formData.floor === "First Floor";
  const ictcSecondFloor =
    ictcHasSpecific && formData.floor === "Second Floor";
  ...
  const requiredNow = useMemo(() => {
    ...
    if (cosHasSpecificRooms) req.push("room"); // COS requires only room, no floor
    return req;
  }, [
    showSubConcern,
    needsOtherConcern,
      ...
      >
        ...
        <main className="create-scope__main">
          ...
          >
            ...
            {metaError && (
              <div className="create-scope__message is-info">
                {metaError}
              </div>
            )}
